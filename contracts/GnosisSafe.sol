    ...
    GuardManager
{
    ...
    // 跟踪所有 批准过的 所有哈希（消息或交易）
    mapping(address => mapping(bytes32 => uint256)) public approvedHashes;
    
    // 此合约只作代理合约的主副本
    constructor() {
        // 通过设置阈值了, 就不能再调用 setup ,  
        // 所以我们创建了一个拥有 0 个所有者和阈值为 1 的保险箱  
        // 这是一个无法使用的保险箱, 非常适合单例
        threshold = 1;
    }
    ...
    /// @param to 可选 委托调用的 合约地址  
    /// @param data 可选 委托调用的 数据负载  
    /// @param fallbackHandler 该合约的 回调处理 
    /// @param paymentToken 用于支付的令牌（0 是 ETH） 
    /// @param payment 支付的金额 
    /// @param paymentReceiver 接收付款的地址（如果 tx.origin 则为 0）
    function setup(
        ...
        // 因为 setupOwners 只有在合约未被初始化的情况下被调用 , 所以我们不需要检查 setupModules
    ...
    /// @dev 允许执行由 所需数量 的所有者确认的安全交易, 然后支付提交交易的帐户  
    /// 注意：即使用户交易失败 , 费用也会转移  
    /// @param to 安全事务的目标地址  
    ...
    /// @param refundReceiver 支付的 gas 接收方地址 （如果 0， 就是 tx.origin ）  
    ...
    function execTransaction(
        ...
        // 我们需要一些 gas 来 在执行后 发出事件（至少 2500）和 一些在执行之前 执行代码（500 gas） 
        // 我们还在检查中包含 1/64 , 它不会与调用一起发送抵消 EIP-150 导致的潜在短路
        require(gasleft() >= ((safeTxGas * 64) / 63).max(safeTxGas + 2500) + 500, "GS010");
        // 这里使用 作用域 来限制变量的 生命周期并防止 `stack too deep` 错误
        {
    ...
    /// @dev 处理付款的函数  
    /// @param gasUsed 用的 gas 数  
    /// @param baseGas 基础 gas 数  
    /// @param gasPrice gas 价格  
    /// @param gasToken gas token 的合约地址  
    /// @param refundReceiver 接收 gas 费的地址  
    function handlePayment(
        ...
        if (gasToken == address(0)) {
            // 对于 ETH , 我们只会将 gas price 调整为不高于实际使用的 gas price
    ...
    */
    ...
    /**
    * @dev 检查提供的签名是否对提供的数据有效  
    * @param dataHash 数据的哈希 （可以是消息哈希或交易哈希） 
    * @param data 应该被签名的数据（这被传递给 外部验证者合约 ） 
    * @param signatures 应该被验证的签名数据， 可以是 ECDSA 签名， 合约签名 (EIP-1271) 或 已批准的哈希  
    * @param requiredSignatures 需要满足的 有效签名的 数量 
    */
        ...
        for (i = 0; i < requiredSignatures; i++) {
            ...
            if (v == 0) {
               ...
               require(uint256(s) >= requiredSignatures.mul(65), "GS021");
                ...
                // 为了支持 eth_sign 和 类似内容 ,  我们调整 v 并在应用 ecrecover 之前使用 以太坊消息前缀对 messageHash 进行散列
                currentOwner = ecrecover(keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", dataHash)), v - 4, r, s);
            } else {
                // 默认是带有提供数据哈希的 ecrecover  
   ...
   function requiredTxGas(
        ...
        // 我们在这里不提供错误消息 , 因为我们使用它来返回 estimateGas
    ...
    */
    ...
    /// @param gasToken 用于支付的代币地址（如果是 ETH , 则为 0）  
    /// @param refundReceiver gas 支付接收方地址（如果是 0x， 则为 tx.origin）  
    /// @param _nonce 事务随机数
    /// @return 事务哈希字节 
